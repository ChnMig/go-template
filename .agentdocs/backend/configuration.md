...
## 概述
项目采用 YAML 配置文件管理配置，支持环境变量覆盖，并在运行时监听配置文件变更进行热重载（重新应用到全局变量）。
...
2. **config/load.go** - 配置加载逻辑
   - `LoadConfig()`：读取配置文件（如存在）并结合默认值与环境变量覆盖
   - `setDefaults()`：设置默认值
   - `applyConfig()`：将 Viper 中的配置应用到全局变量
   - 环境变量前缀：`HTTP_SERVICES`，示例：`HTTP_SERVICES_SERVER_PORT=9090`
...
4. **main.go** - 配置初始化流程
   - 加载配置 → 设置运行模式 → 初始化 logger → 校验配置 → 启动服务

## 使用规范

### 添加新配置项
1. 在 `config.yaml` 和 `config.yaml.example` 中添加配置项
2. 在 `config/config.go` 中定义全局变量
3. 在 `config/load.go` 的 `setDefaults()` 中设置默认值
4. 在 `config/load.go` 的 `applyConfig()` 中读取并解析到全局变量
...
## 依赖
- `github.com/spf13/viper` - 配置加载（默认值 + YAML + 环境变量）
- `github.com/fsnotify/fsnotify` - 文件变更监听（Viper 内部使用，部分模块也复用）
...
- 热重载会重新应用到全局变量；但端口等启动期配置仍需要重启服务才能生效
